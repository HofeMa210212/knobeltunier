...
import 'package:knoppeltunier/match_player.dart';
import 'package:knoppeltunier/player.dart';
import 'package:knoppeltunier/widgets/widgets.dart';
...
class Tournament{
  ...
  Future<void> fillWithDemoPlayers() async {
    ...
    while (remainingTickets > 0) {
      ...
      try {
        await addPlayer(p, this.id); // Spieler hinzufügen
...
// Hilfsfunktion zur Erstellung eines eindeutigen Schlüssels für ein Spielerpaar
  ...
  Future<void> generateMatchTimes(DateTime startTime, int matchTimeLength) async {
    ...
    for(Match m in _matches){
      await addMatchDb(db, m.player1.id, m.player2.id, m.place!, m.time.toString(), this.id);
    }

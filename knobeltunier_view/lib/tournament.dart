...
class Tournament{
  ...
  Future<void> addPlayer(Player p, int tournamentId) async {
    ...
    if (!_players.contains(p)) {
      ...
      throw Exception('Player ${p.fName} ${p.lName} is already in the tournament.');
    }
  }
  ...
  Future<void> addMatchPlace(MatchPlace mP, int tournamentId) async {
    ...
    if (!_places.contains(mP)) {
      _places.add(mP);
      addPlaceDb(db, mP.matchLength, mP.name, mP.startime.toString(), id);
  ...
  Future<void> fillWithDemoPlayers() async {
    Random r = Random();
    ...
    while (remainingTickets > 0) {
      ...
      try {
        await addPlayer(p, id); // Spieler hinzufügen
  ...
  Future<void> createMatches(m.TimeOfDay startTime) async {
  ...
  void nextRound(){
    ...
    if(allMatchesPlayed()){
      ...
      _matches = _createMatches(matchPlayers);
      generateMatchTimes(DateTime.now());
  ...
  Future<void> generateMatchesPdf() async {
    ...
    if(filePath != null){
      filePath = "$filePath/${name}_matches.pdf";
  ...
  Future<void> generateMatchTimes(DateTime startTime) async {
    Database db = await dbController();
    int matchTimeLength = 2;
    ...
    await executeQuery(db, "DELETE FROM match WHERE tournamentId=$id");

    for(Match m in _matches){
      await addMatchDb(db, m.player1.id, m.player2.id, m.place!, m.time.toString(), id);
    }
  ...
  Future<void> getMatchesFromDb() async{
    ...
    List<Map<String, dynamic>> newMatchData = await executeQueryServer('''
    SELECT 
    match.id AS matchId,
    match.placeName,
    match.startTime,
...
Future<List<Tournament>> getTournamentsFromDb() async{
  ...
  List<Map<String, dynamic>> tournamentsRaw = await executeQueryServer("SELECT * FROM tournament");
